...
def _zero_metrics() -> dict:
    m = dict(
        ...
        successful_door_opens=0,    # toggle that opened the door (reward > 0)
...
def _extract_metrics(game, buffer, n_traj, mean_losses, itr) -> dict:
    ...
    try:
        ...
        if tp_arr.ndim == 2 and tp_arr.shape[1] > 1 and len(act_arr) == len(tp_arr):
            ...
            m["successful_door_opens"]       = int((rew_arr > 0).sum())  # reward>0 ⟺ door opened
            ...
            fb = {"key_never_found": 0, "key_visible_but_navigation_failed": 0,
                  ...
                  "timeout_failure": 0}
            for s, e in zip(traj[:-1], traj[1:]):
                if float(rew_arr[s:e].sum()) > 0:   # success → not a failure
                    continue
                key_det = key_pick = False
                for t in range(s, e):
