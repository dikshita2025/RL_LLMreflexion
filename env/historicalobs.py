...
def _shallow_grid_copy(original: Grid) -> Grid:
    """
    BUG FIX: O(W*H) shallow list copy instead of deepcopy.

    Grid.copy() calls deepcopy(self) which recursively copies every
    WorldObj in the grid — catastrophically slow when training adds
    reflection memory, trajectory buffers, and planner state.

    WorldObj instances are NEVER mutated during encode()/render() — only
    the single agent-position cell is overwritten (via grid.set()) after
    this copy is made.  Sharing WorldObj references is therefore safe.
    """
    new = Grid.__new__(Grid)
    new.width  = original.width
    new.height = original.height
    new.grid   = list(original.grid)   # shallow: reference copy of WorldObj ptrs
    return new
...
class HistoricalObsEnv(MiniGridEnv):
    ...
    def reset(self, seed=None, options=None):
        # options is forwarded to MiniGridEnv.reset() for gymnasium 1.x compatibility
    ...
    def gen_hist_obs_grid(self, agent_view_size=None):
        ...
        # BUG FIX: use shallow list copy instead of deepcopy (see _shallow_grid_copy)
        grid = _shallow_grid_copy(self.grid)
        vis_grid = self.slice_grid(topX, topY, botX - topX, botY - topY)
        if not self.see_through_walls:
            vis_mask = vis_grid.process_vis((self.agent_pos[0] - topX, self.agent_pos[1] - topY), self.agent_dir)
        else:
