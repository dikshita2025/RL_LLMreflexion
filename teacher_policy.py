...
class FailureDetector:
    ...
    def is_making_progress(self, agent_pos) -> bool:
        """
        Return True if the agent has made any movement progress recently.
        Used to suppress interventions when the agent is slowly moving toward target.
        """
        if agent_pos is None:
            return True   # can't tell — don't intervene
        dist = float(np.linalg.norm(np.array(agent_pos).astype(float)))
        self._progress_window.append(dist)
        if dist < self._min_dist_seen:
            self._min_dist_seen = dist
            return True   # improving — definitely progress
        # Progress = any improvement in last 30 steps
        if len(self._progress_window) >= 10:
            return min(self._progress_window) < max(self._progress_window)
        return True
...
class TeacherPolicy:
    ...
    # ── Public API for reflection system ──────────────────────────────────────
    ...
    def _handle_intervention(self, obs_text: str, reason: str):
        ...
        if mediator is not None and hasattr(mediator, "set_failure_context"):
            mediator.set_failure_context(
                f"Previously {reason} at this state. Try a different approach."
            )
